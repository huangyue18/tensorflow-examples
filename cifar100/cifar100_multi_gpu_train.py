...
"""A binary to train CIFAR-100 using multiple GPUs with synchronous updates.

Accuracy:
cifar100_multi_gpu_train.py achieves ~86% accuracy after 100K steps (256
epochs of data) as judged by cifar100_eval.py.
    ...
    with tf.variable_scope(tf.get_variable_scope()):
      for i in xrange(FLAGS.num_gpus):
        with tf.device('/gpu:%d' % i):
          with tf.name_scope('%s_%d' % (cifar100.TOWER_NAME, i)) as scope:
